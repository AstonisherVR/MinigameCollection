...
@onready var snake_head_segment: SnakeBodySegment = %"Snake Head Segment"
@onready var body_parts: Array[SnakeBodySegment] = [snake_head_segment]	# Store the segment instances
...
const SNAKE_BODY_SEGMENT := preload("res://Player/Snake Player/snake_body_segment.tscn")
...
func _update_snake_body_movement() -> void:
	#TODO TODO TODO TODO FIXME FIXME BUG BUG
	# Make each part of the snake a part of an array nad move them individually
	body_parts[0].position += direction * grid_size 
...
func _update_rotation() -> void:
	if (direction.x + 2 * direction.y) * 90 != -180:
		body_parts[0].rotation_degrees = (direction.x + 2 * direction.y) * 90
	else: body_parts[0].rotation_degrees = 0
...
func _update_predicted_position() -> void:
	predicted_position = body_parts[0].position + predicted_direction * grid_size
