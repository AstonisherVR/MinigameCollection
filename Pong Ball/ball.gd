...
@onready var current_speed: float = initial_speed
# The ball's current direction - set randomly at start
@onready var direction: Vector2 = _random_start_direction()
...
func _ball_movement(delta: float) -> void:
	...
	var collision: KinematicCollision2D = move_and_collide(direction * current_speed * delta)
	# If we hit something...
	if collision:
...
func _calculate_bounce_direction(collider: PhysicsBody2D) -> Vector2:
	...
	var distance: float = ball_y_pos - paddle_y_pos
	...
	var collision_shape_size: Vector2 = collision_shape.shape.get_rect().end
